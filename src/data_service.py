...
class DataService():
    ...
    def dataset_features_provider(self, identity, translator, dataset, write):
        ...
        if permissions:
            self.logger.debug(f"Have permissions for identity {identity} on dataset {dataset} with write={write}")
            dataset_features_provider = DatasetFeaturesProvider(
                permissions, self.db_engine, self.logger, translator
            )
        else:
            self.logger.debug(f"NO permissions for identity {identity} on dataset {dataset} with write={write}")
