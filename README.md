...
- Starting with qwc-map-viewer v2022.05.17, localized translation forms are supported. To this end, place a Qt Translation file called `<form_basename>_<lang>.ts` next to the designer form `<form_basename>.ui`, where `lang` is a language or language/country code, i.e. `en` or `en-US`. There is a script to help generate the translation files at [`scripts/translateui.sh`](https://github.com/qwc-services/qwc-data-service/blob/master/scripts/translateui.sh). Example:
